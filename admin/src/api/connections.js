 ...
 * @param {AxiosRequestConfig} axiosConfigs - Axios config options for making requests
 */
export async function fetchInspectors(context, type, axiosConfigs) {
    axiosConfigs = {
        ...axiosConfigs,
        baseURL: 'http://localhost:3003/',
    };
    ...
    try {
        const response = await axios.request(axiosConfigs);
        context.commit({
            type,
